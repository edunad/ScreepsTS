...
import { TravelToOptions, HasPos, Coord, PathfinderReturn, TravelData, TravelState, TravelToReturnData } from "./TravelerInterfaces";
...
export class Traveler {
    ...
    public static travelTo(creep: Creep, destination: HasPos|RoomPosition, options: TravelToOptions = {}): number {
        ...
        if (options?.style) {
            const tmpPath = creep.memory?._trav?.path;
            if (tmpPath) {
                ...
                for (let i = 1; i < tmpPath.length && i < 20; i++) {
                    ...
                    line.line(curX, curY, nextX, nextY, options.style);
                    curX = nextX;
                    curY = nextY;
                }
            }
