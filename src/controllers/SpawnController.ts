...
export class SpawnController {
    ...
    private static templates: {[key in CreepRole]: {[id: number]: CreepParts[]}} = {
        ...
        'Attacker': {
            1: [ATTACK, ATTACK, ATTACK, MOVE, MOVE, MOVE, MOVE],
        }
    }
    ...
    public static run(spawnID: Spawns = "Spawn1"): void {
        ...
        const spawnConfig = SettingsController.get(spawn.room.name, "SPAWNS");
        ...
        for(let i: number = 0; i < spawnEntries.length; i++) {
            ...
            for(let indx: number = 0; indx < levels.length; indx++) {
                ...
                if(totalSpawned.length < amountWanted) {
                    ...
                    const resp = spawn.spawnCreep(template[level], `${role.toUpperCase()}-${uniqueNamesGenerator({dictionaries: [adjectives, animals], style: 'upperCase'})}`, {memory: {role: role, level: level}});
